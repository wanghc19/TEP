...
pars2.M = 80;
pars2.L = 4;
er = 13; % relative permittivity of the interior medium
nref = sqrt(er);
d = 1.0; % periodicity
beta_scan = linspace(0.01, 0.5, 50)*d/2/pi;
bands = zeros(1, length(beta_scan));
[C,curvelen,~,~] = LOCAL_construct_cont(ntot,flag_geom,0,0);
for i = 1:length(beta_scan)
  beta = beta_scan(i);
  pars1.beta = beta;
  pars1.d = d;
  k_scan = linspace(eps,beta-eps,50);
  smin = 1e10;
  ...
  bands(i) = kh_min;
  fprintf(1,'beta = %3.5e, kh = %3.5e, min singular value = %3.5e\n', beta, kh_min, smin);
end

figure;
plot(beta_scan, bands, 'b.-');
xlabel('Bloch wavenumber \beta'); ylabel('Eigenvalue kh');
