...
| ID | Category | Open problem | Blocking scope | Cheapest next check | Status |
|---|---|---|---|---|---|
| OP-CI3-1 | `BLOCKER` | I3.1 已得到四个有效负结果：`center-a1` 受固定单胞 cutoff 主导；`lead-a3` 在 fixed BIE-informed fit 停止；Q1--RT0 `weak-a1` 在 fine phase/scale Gram qualification 停止；BIE-collar `bie-a3` 则通过 finite input、branch/Wood、propagation、density、surface 与 safe-field 门，但在 coarse lead 的 composite RT0-majorant 预因子处以 `HDIV_FLUX_UNRESOLVED` 停止。该矩阵已含 true-circle polar correction，而 integration-object gate 尚未先行检查；没有形成 flux、majorant、tail、indicator 或 interval。 | 当前最近的 blocker 是含材料圆修正的 RT0-majorant quadrature/assembly 尚未形成可审计、保持正性的离散对象。它阻止 continuous residual estimator candidate 与 I3.2 输入，但不撤销 `bie-a3` 首败前的 BIE safe-field 内部资格，也不证明 $H(\mathrm{div})$ 理论失败。之后仍有 RT0 continuity、tail、mesh、phase/scale、width 与可靠 enclosure 门。finite zero、score minimizer 和 production derivative 不是主线 blocker。 | 新 revision 先在 factor 前保存 Q1 record，以及 lead/center 的 $L_A$、background $L_B$、disk correction 和 final free-block diagonal 的 finite/nonpositive count、minimum 与坏 edge；先判 integration/composite positivity，再判 factor。不得 clipping、正则化、调阈值或追溯修改 `bie-a3`。只有分清 quadrature positivity 与 assembly/index error 后，才另行冻结最小修订。 | `OPEN` |
