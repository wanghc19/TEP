...
| ID | Category | Open problem | Blocking scope | Cheapest next check | Status |
|---|---|---|---|---|---|
| OP-CI3-1 | `BLOCKER` | I3.1 已选择 saved-candidate global continuous weak residual，但 continuous Hilbert/form spaces、确定性 conforming field reconstruction、完整 residual decomposition、computable dual norm 和 numerical allowance 尚未闭合。 | 这些对象/可计算性门不阻止继续 I3.1 理论，但阻止创建当前实验设计。finite zero、common matrix transport 和 production derivative 不再是主线 blocker。 | 先冻结 continuous form、field repair/cutoff、residual components 和 Riesz dual-norm approximation；在未来 design 中预注册 manufactured field、jump/tail negatives 和 Riesz refinement。 | `OPEN` |
