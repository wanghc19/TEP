...
| ID | Category | Open problem | Blocking scope | Cheapest next check | Status |
|---|---|---|---|---|---|
| OP-CI3-1 | `BLOCKER` | 四条历史路线已给出有效负结果；纯 BIE `pbie-a2` 进一步计算出 $q=1.1049370224693775\times10^{-10}$ 和宽度 $4.0501912934587381\times10^{-10}$ 的名义区间，但它仍为 `NUMERICALLY_UNQUALIFIED`。正式 warnings 为 wall $256\to512$ change $0.23020558465752644$、nonzero-mode $T$ manufactured error 至多 $1.4438757363102721$、circle outside-$M$ share $0.51468601513057144$。 | 三项内部数值资格未闭合，阻止冻结 estimator specification 并交给 I3.2 独立经验验证；它们不撤销 $q$、full-$P$ tail、phase/scale 和名义代数变换的普通双精度复算结果。outward enclosure、projected gap 与宽度门另属 OP-CI3-2/I3.3，不是 I3.2 前置条件。`pbie-a1` 只是 warning-schema implementation failure；finite zero、score minimizer、RT0 repair 和 production derivative 都不再是当前主线 blocker。 | 先核对实际 $T(k_{\mathrm{out}})-T(k_{\mathrm{in}})$ difference block，以区分单个 $T$ oracle 与实际 operator action；再对 wall action 与 circle outside-$M$ 表示做冻结 refinement qualification。三项闭合后即可冻结 estimator 供 I3.2 独立验证；不得调 warning 阈值、追溯改 output 或把同链检查称独立 reference。 | `OPEN` |
