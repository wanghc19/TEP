...
graph-basis mutation 门仍未过，production derivative 仍不可用。
在本节的 continuous/root blockers 关闭前，仍不得运行 contour 或 root isolation，也不得作真实 eigenvalue 声明。现行方法见
...
| OP-M0-3 | `BLOCKER` | continuous one-cell relation 到 BIE/Fourier generalized pencil 的 primal/adjoint $C^1$ consistency 仍未证明，production `DIF/sep` backend 和 analytic graph tangent 也未资格化。有限维设计已冻结 original/reversed 两次 QZ、regular infinite pair、seed-cluster continuation、双向 Sylvester separation、absolute chart margin、Dirichlet/Robin/full-graph fallback 和 $A_{\mathrm{def}}^{D/G}$；I1.2 static arms 与 I1.3 real-$k$ projector/chart 连续性均已通过，但 FD graph-basis mutation 为 $3.65\times10^{-11}>10^{-12}$。 | 不阻止冻结点的经验矩阵解释、离散 candidate 或 I1.4 readiness 设计；阻止把有限维 graph/DtN 称为带扰动界的 production approximation，也阻止 production $A_{\mathrm{def}}'$、Newton correction、真实 root/eigenvalue 和 theorem-level estimator 声明。 | I1.4 先实现 anchored complex-$k$ cluster continuation 与固定 chart；Newton 前再冻结 roundoff-aware derivative mutation test，必要时资格化 structured `xTGSEN/xTGSYL` condition estimate 和 continuous $C^1$ bridge。 | `OPEN` |
...
| OP-CI1-2 | `BLOCKER` | 已有 fixed-$M=48$ 实轴离散候选 $k=1.8327703475952146$，但新模型尚无其 selected complex-$k$ disk 上的 anchored Rayleigh branch、seed-cluster continuation、fixed chart/rank、factor/pole ledger、full-$F$ CR 和完整实际-object negatives。当前 real-$k$ runner 仍逐点按 modulus 重选 QZ cluster，只用 overlap 作事后检查。 | 直接阻止 contour count、locator 结论和 bordered Newton root isolation；不否定当前离散实轴 candidate。 | 只有另行授权 I1.4 后，才围绕该候选预注册小复圆盘及 anchored cluster、固定 chart/rank、pole/branch、CR 与 anti-collapse 门。 | `SCHEDULED` |
...
| OP-CI1-4 | `MINOR CAVEAT` | $M=48$ regular-infinite count 在 coarse/fine pass 间为 87--88，但始终位于 97 维 unstable complement。v2 的 33 点均为 97 stable / 97 unstable、0 neutral / 0 indeterminate，最小相邻 overlap 为 $0.9999959813$，未见 selected subspace 不连续。旧 swapped-wall 负例仍主要由 chart-unavailable fail-closed 检出。 | 不阻止当前 candidate 或 I1.4 readiness；只限制对 complementary infinite classification 和可解 swapped chart 的定量描述。 | complex-$k$ disk 继续逐点记录 infinity/count/overlap；只有 selected cluster 不连续时再升级。 | `RESOLVED` |
| OP-CI1-5 | `IMPORTANT CAVEAT` | 本轮按经验目标未形成或施加 production generalized-Sylvester separation operator；`production_separation=NOT_EVALUATED_CAVEAT`，chart 只认证代数 margin/condition/solve，不是 perturbation-certified。 | 本项本身不阻止冻结点 static I1.2、经验型参数连续性、candidate reconnaissance 或 I1.4 readiness 设计；阻止 theorem-level graph/DtN conditioning bound、certified root 和无条件 estimator。 | 若 I1.4 另行获批，继续使用廉价 count/overlap/chart continuity 门；只有出现 branch ambiguity 或论文 claim 升级时，才实现 structured `xTGSEN/xTGSYL` condition estimate。 | `OPEN` |
...
| ID | Category | Open problem | Blocking scope | Cheapest next check | Status |
|---|---|---|---|---|---|
| OP-CI2-1 | `BLOCKER` | 已有 fixed-$M=48$ 离散实轴候选 $k=1.8327703475952146$，但尚无 count-one contour、converged bordered Newton root、simple-root derivative 和跨层 root matching。 | 阻止把候选称为 eigenvalue，也阻止计算有意义的后验 correction。 | 另行授权并完成 I1.4 complex-disk readiness 后，才可对预注册 disk 做 contour count、Newton 和相邻两个 levels 的 root matching。 | `OPEN` |
