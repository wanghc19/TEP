...
direct $M=48$ static QZ/graph/DtN/$A_{\mathrm{def}}$ 链也已通过。经验型 I1.3 参数连续性、
$A_{\mathrm{def}}'$ 开发和 candidate reconnaissance 可以推进；在本节的 continuous/root
blockers 关闭前，仍不得运行 contour 或 root isolation，也不得作真实 eigenvalue 声明。现行方法见
...
| OP-M0-3 | `BLOCKER` | continuous one-cell relation 到 BIE/Fourier generalized pencil 的 primal/adjoint $C^1$ consistency 仍未证明，production `DIF/sep` backend 和 analytic graph tangent 也未资格化。有限维设计已冻结 original/reversed 两次 QZ、regular infinite pair、seed-cluster continuation、双向 Sylvester separation、absolute chart margin、Dirichlet/Robin/full-graph fallback 和 $A_{\mathrm{def}}^{D/G}$；I1.2 的 manufactured、$M=5,8$ exact-small-sep 和 direct $M=48$ sep-free static arms 已通过。 | 不阻止冻结点的经验矩阵解释、经验型 I1.3 或 candidate reconnaissance；阻止把有限维 graph/DtN 称为带扰动界的 production approximation，也阻止 certified $A_{\mathrm{def}}'$、真实 root/eigenvalue 和 theorem-level estimator 声明。 | 先用最小 real-$k$ stencil 验证 projector/chart 连续性并开发经验型 tangent；只有更强 claim 需要时再资格化 structured `xTGSEN/xTGSYL` condition estimate 和 continuous $C^1$ bridge。 | `OPEN` |
...
| OP-CI1-3 | `IMPORTANT CAVEAT` | direct $M=48$ input 已记录 MATLAB `lsqminnorm`、Git、generator/config/runner/package hashes 和 zero-fallback/rank fields；静态源码扫描也未发现 `pinv` 或 solver switch，但 zero fields 仍主要是 generator self-attestation，不是独立 runtime solver trace。 | 不阻止 I1.2 或经验型 I1.3；限制对 solver path 的更强审计声明。 | 若候选对 solver 细节敏感，再加受控 shadow negative 或 runtime call ledger；否则保留 source hashes 和 caveat。 | `SCHEDULED` |
| OP-CI1-4 | `MINOR CAVEAT` | $M=48$ 正向 amplitude charts 已记录 $rcond(D)$、$rcond(F)$ 和约 $10^{-15}$ fixed-point residual；regular-infinite count 在 coarse/fine 的一个 pass 上为 87/88，但始终位于 97 维 unstable complement，selected projector 变化约 $10^{-14}$。旧 swapped-wall 负例仍主要由 chart-unavailable fail-closed 检出。 | 不阻止方向、静态 subspace 或经验型 I1.3；只限制对 complementary infinite classification 和可解 swapped chart 的定量描述。 | I1.3 保存逐点 infinite count、stable projector overlap 和 reference residual；只有 selected subspace 不连续时才升级。 | `SCHEDULED` |
| OP-CI1-5 | `IMPORTANT CAVEAT` | 本轮按经验目标未形成或施加 production generalized-Sylvester separation operator；`production_separation=NOT_EVALUATED_CAVEAT`，chart 只认证代数 margin/condition/solve，不是 perturbation-certified。 | 不阻止冻结点 static I1.2、经验型参数连续性、经验型 $A_{\mathrm{def}}'$ 或 candidate reconnaissance；阻止 theorem-level graph/DtN conditioning bound、certified root 和无条件 estimator。 | I1.3 先用廉价 projector/chart continuity screen；只有数据出现 branch ambiguity 或论文 claim 升级时，才实现并审查 structured `xTGSEN/xTGSYL` condition estimate。 | `OPEN` |
