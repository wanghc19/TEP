...
| ID | Category | Open problem | Blocking scope | Cheapest next check | Status |
|---|---|---|---|---|---|
| OP-CI2-1 | `BLOCKER` | I2.1 已通过 factor-aware count one，I2.2 又得到冻结 $H_{\mathrm{sym}}$ 两端的稳定 single sign-count difference，但尚未在预先冻结的不同离散阶数下得到同一物理 mode 的可比较 candidate 序列、漂移与定位不确定度。 | 阻止 I2.3 向 I3 提供可解释的 cross-discretization 数据；不阻止设计 I2.3。 | 冻结离散阶数、单一 refinement axis、candidate 定义与公共 mode 表示，报告 signed/absolute drift、各层定位 uncertainty 及最低 residual/factor/field/boundary/mode-identity 检查；若漂移不能与组合 uncertainty 分离，则记 `DRIFT_UNRESOLVED`。无需先证明 exact finite root。 | `OPEN` |
