...
| ID | Category | Open problem | Blocking scope | Cheapest next check | Status |
|---|---|---|---|---|---|
| OP-CI3-1 | `BLOCKER` | `center-a1` 已对一个真正属于 continuous operator domain 的中心空列紧支撑场计算 strong-residual ratio，但结果为 $22.43882099031153$，且固定单胞 cutoff 的两个导数项主导。名义区间跨过零，远不能达到预注册 $10^{-6}$ 频率分辨率。尚无兼具 continuous conformity 与有用分辨率的 estimator candidate。 | 阻止把当前 ratio 移交 I3.2 或称为 continuous-eigenvalue error estimator；不撤销 compact-support trial field、strong-residual 公式和可复现负结果，也不阻止继续 I3.1。finite zero、common matrix transport 和 production derivative 仍不是主线 blocker。 | 另行冻结一个 cutoff defect 更小的 lead-aware conforming reconstruction，或回到含全部 defect 的 continuous weak residual。先验证其信号达到预注册尺度；不要优先为同一宽 baseline 增加严格积分 enclosure 或 projected-gap 认证。 | `OPEN` |
