...
| OP-I4-1f | `BLOCKER` | full SLP--D 三路径已认证：E--P/P--R 最大系数误差 $4.33\times10^{-10}$，P proxy self 最大 $3.97\times10^{-10}$。SLP--N 的 E--P/P--R 最大误差 $5.58\times10^{-9}$、E--R $9.61\times10^{-16}$，且 $n_{\mathrm{tot}},N_y$、Ewald、tail、$M_{\mathrm{pw}}$ gates 通过；唯一失败是 P/$G_x$ 对 Nside、Ntop、Nedge 的 self changes $6.56\times10^{-9}$、$2.58\times10^{-9}$、$1.09\times10^{-8}$。当前分类为 `SLP_N_UNCERTIFIED_P_GX_PROXY_SELF_CONVERGENCE`。 | 直接阻止 full SLP--N、后续 DLP--D/DLP--N、certified $M_{\mathrm{trace}}$、DtN locator 和 root isolation；不再阻止 SLP--D claim。 | 以 joint $(160,160,80,32)$ 为候选 authority，只重算 P/SLP--N，依次单轴测试 Nedge 96、Nside 200、Ntop 200、$M_{\mathrm{pw}}=40$；每项 self $\le2\times10^{-9}$ 且 joint coefficient triangle仍 $\le10^{-8}$，首项失败即停。若失败，再在 joint 与失败 level 固定各自 $A,b$ 做 explicit-`lsqminnorm` rank audit；不得静默替换 public solver。 | `OPEN` |
| OP-I4-1h | `BLOCKER` | test-local Ewald analytic gradient/Hessian 的 point reference 已认证。full SLP--D 已通过；full SLP--N 因 OP-I4-1f 的 P/$G_x$ proxy self 失败。DLP--D/DLP--N 按冻结顺序未运行。另有独立下游证据：P Hessian hold-out 误差 $1.46\times10^{-8}$--$1.60\times10^{-8}$，fixed-$A,b$ solver spread $3.04\times10^{-8}$，而 same-P Hessian formula 自洽到 $2.12\times10^{-11}$。 | Ewald derivative 本身不再是 blocker；未完成的 wall Cauchy-data 与 DLP--N solver-rank 门仍阻止 $M_{\mathrm{trace}}$ 和 locator。 | OP-I4-1f 关闭后先运行 DLP--D；只有 DLP--D 通过才对 DLP--N 做 explicit-`lsqminnorm` tolerance/rank qualification并重启 wall action。前一项失败即停止。 | `SCHEDULED AFTER OP-I4-1f` |
...
| OP-I4-6 | `IMPORTANT CAVEAT` | 三路径实验的 Rayleigh retained-row change 为 0，因为 extractor 对每个 $m$ 独立构造；扩大输出 $M$ 不改变已存在的 row。 | 不影响 Ewald--Rayleigh 对当前 SLP--D 系数的条件通过，但阻止把 retained-row equality 解释成 Rayleigh tail convergence 或 $M_{\mathrm{trace}}$ 认证。 | OP-I4-1f 与其余 wall actions 闭合后，用 Ewald full wall field 分别测 $M=12,24,48$ 的重构误差和 omitted coefficient energy；不要再用 retained-row identity 代替 tail test。 | `SCHEDULED` |
