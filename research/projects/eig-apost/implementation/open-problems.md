...
| ID | Category | Open problem | Blocking scope | Cheapest next check | Status |
|---|---|---|---|---|---|
| OP-CI3-1 | `BLOCKER` | I3.1 已得到三个有效负结果：`center-a1` 的 ratio $22.43882099031153$ 由固定单胞 cutoff 主导；`lead-a3` 在 fixed BIE-informed fit 停止；Q1--RT0 `weak-a1` 的 base finite input、propagation、Q1--RT0、Gram 与 full-$P$ tails 通过，但 fine phase/scale repeat 的 center 的 $A,B$ Gram 及左右 first-cell 的 $A$ Gram Hermitian defect 最大为 $6.2442\times10^{-10}>10^{-12}$，故在 `MAJORANT_QUADRATURE_UNRESOLVED` 停止。base coarse/fine $B/\gamma$ change 约 $0.277>0.20$，fine nominal width 约 $0.564\gg10^{-6}$，但 refinement/estimator 未到达。 | 当前直接阻止 continuous residual estimator candidate 和 I3.2 输入的是 scale-covariant Gram qualification；即使关闭，mesh qualification 和有用 interval width 仍是下一层 blocker。不撤销三个 valid negative，也不撤销 `weak-a1` 首败前通过的 base Q1--RT0/tail checks。finite zero、score minimizer、common matrix transport 和 production derivative 仍不是主线 blocker。 | 先对 normalized per-cell Gram 的 phase/scale covariance 做独立、低成本的代数/舍入检查，再另行冻结任何 Gram assembly、oracle 或 threshold 修订；不得跳过首败或追溯修改 `weak-a1`。关闭后仍按原顺序检查 mesh component 与 absolute width，不得把 Gram 修复等同于 estimator 成功。 | `OPEN` |
