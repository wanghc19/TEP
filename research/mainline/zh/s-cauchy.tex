...
\begin{equation}
  \mathcal E^\pm(k,\beta)
  :=\left\{\Tr^{0,\pm} w:w\in
  \mathcal U^\pm(k,\beta)\right\}
  \subset\mathcal H(\Gamma^\pm).
...
\begin{lemma}[公共本质谱带隙排除传播乘子]
\label{lem:gap-unit-circle}
假设理想波导在横向周期方向上的 Floquet 变换有效，且非零解迹对应非零 Bloch 解。若 $k^2\in I_\beta$，则
\begin{equation}
...
\begin{proof}
若存在模为一的乘子，可将其写为 $\lambda=e^{\ii\theta L^\pm}$，其中 $\theta$ 是实横向准动量。相应非零解满足单胞元横向 Bloch 条件，并使 $k^2$ 属于理想波导的 $\theta$-纤维谱。Floquet 直积分分解随即给出 $k^2\in\sigma(A^\pm(\beta))\subset\Sigma_{\mathrm{ess}}(\beta)$，这与 \eqref{eq:strict-gap} 矛盾。这正是公共本质谱带隙中不存在传播半波导通道的谱意义。
\end{proof}
