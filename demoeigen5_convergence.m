% Convergence study for sigma_min near one candidate eigenvalue.
format long;
clear;
...
results = LOCAL_run_ntot_convergence_study(flag_geom, iprec, nref, pars1, pars2, ...
  ntot_list, k_interval_initial, nrefine, ngrid_refine, k_fixed_ref, ...
  flag_print_level_details);

LOCAL_print_convergence_summary(results, k_fixed_ref);
...
function results = LOCAL_run_ntot_convergence_study(flag_geom, iprec, nref, pars1, pars2, ...
    ntot_list, k_interval_initial, nrefine, ngrid_refine, k_fixed_ref, flag_print_level_details)
  ...
  for j = 1:ncases
    ...
    [record, history] = LOCAL_refine_interval_for_one_ntot(ntot, geometry_cache, nref, iprec, ...
      pars1, pars2, k_interval_initial, nrefine, ngrid_refine);
...
function LOCAL_print_convergence_summary(results, k_fixed_ref)
  ...
  fprintf('\nConvergence summary:\n');
  if isempty(k_fixed_ref)
    fprintf(['  %-8s %-22s %-20s %-22s %-22s %-16s %-16s\n'], ...
      'ntot', 'k_best', 'sigma_best', 'interval_left', 'interval_right', ...
      'width', '|k_best-k_inf|');
  else
    fprintf(['  %-8s %-22s %-20s %-22s %-22s %-16s %-20s %-16s\n'], ...
      'ntot', 'k_best', 'sigma_best', 'interval_left', 'interval_right', ...
      'width', 'sigma_fixed', '|k_best-k_inf|');
  end

  for j = 1:length(records)
    record = records(j);
    if isempty(k_fixed_ref)
      fprintf(['  %-8d %-22.16f %-20.12e %-22.16f %-22.16f %-16.8e %-16.8e\n'], ...
        record.ntot, record.k_best, record.sigma_best, record.interval_left, ...
        record.interval_right, record.interval_width, abs(record.k_best - k_inf));
    else
      fprintf(['  %-8d %-22.16f %-20.12e %-22.16f %-22.16f %-16.8e %-20.12e %-16.8e\n'], ...
        record.ntot, record.k_best, record.sigma_best, record.interval_left, ...
  ...
  fprintf('\nDiagnostics:\n');
  fprintf('  k_inf (largest ntot) = %.16f\n', k_inf);
...
function LOCAL_print_one_ntot_progress(record)
  ...
  fprintf(['\nCompleted ntot = %d: k_best = %.16f, sigma_best = %.12e, ', ...
    'final width = %.8e\n'], ...
    record.ntot, record.k_best, record.sigma_best, record.interval_width);
...
function LOCAL_plot_convergence_results(results, flag_plot_sigma, flag_plot_kbest, flag_plot_kerror)
  ...
  if flag_plot_kerror
    ...
    if any(valid_error)
      ...
      title('Convergence of refined minimizer location', 'FontSize', 12);
    else
