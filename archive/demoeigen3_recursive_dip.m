...
function A = LOCAL_construct_A(C, iprec, khint, pars1, proxy, curvelen)
  ...
  A11 = (gradx_int .* repmat(ny1, ntot, 1) + grady_int .* repmat(ny2, ntot, 1)) .* weight_mat;
  A12 = pot_int .* weight_mat;
  A21 = (hessxx_int .* nx1ny1 + hessxy_int .* nx1ny2_nx2ny1 + hessyy_int .* nx2ny2) .* weight_mat;
  A22 = (gradx_int .* nx1mat + grady_int .* nx2mat) .* weight_mat;

  src_geom = [x; y];
  for iy = 1:ntot
    tgt_idx = [1:(iy - 1), (iy + 1):ntot];
    u = qpgreen_mfs(src_geom(:, iy), src_geom(:, tgt_idx), pars1, proxy);

    col_weight = src_weight(iy);
    A11(tgt_idx, iy) = A11(tgt_idx, iy) - (ny1(iy) * u.grad(1, :) + ny2(iy) * u.grad(2, :)).' * col_weight;
    A12(tgt_idx, iy) = A12(tgt_idx, iy) - u.pot.' * col_weight;
    A21(tgt_idx, iy) = A21(tgt_idx, iy) - ...
      (nx1(tgt_idx) * ny1(iy) .* u.hess(1, :).'+ ...
       (nx1(tgt_idx) * ny2(iy) + nx2(tgt_idx) * ny1(iy)) .* u.hess(2, :).'+ ...
       nx2(tgt_idx) * ny2(iy) .* u.hess(3, :).') * col_weight;
    A22(tgt_idx, iy) = A22(tgt_idx, iy) - ...
      (nx1(tgt_idx) .* u.grad(1, :).' + nx2(tgt_idx) .* u.grad(2, :).') * col_weight;
  end
