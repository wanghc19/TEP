% Purpose:
%   Variant of tep_scan_local4_1.m for testing all four Muller blocks with
%   Kress assembly.
%
% Main algorithm:
%   Keep the recursive local k-scan workflow from tep_scan_local4_1.m.
...
%
% Main changes:
%   LOCAL_construct_A now assembles A11 and A22 using Kress split kernels
%   L and L*.  The S and Tdiff Kress paths are inherited from
%   tep_scan_local4_1.m.
%
% Numerical goal:
%   Test the local singular-value scan when all four integral blocks use
%   Kress assembly.

function tep_scan_local4_2
...
% --- 1. Parameter Setup ---
ntot = 40;                            % Single boundary point count for dip refinement
...
max_refine_level = 4;
% initial_interval = [2.65350974, 2.65351408]; % for ellipse
initial_interval = [2.13012810, 2.13012840]; % for star
...
function [A11, A22] = LOCAL_assemble_D_blocks_kress(C, kin, kout, ...
  ...
  [L1_in, L2_in, Ls1_in, Ls2_in, L_res_in, Ls_res_in] = ...
    LOCAL_double_layer_splits(kin, t, geom_data);
  [L1_out, L2_out, Ls1_out, Ls2_out, L_res_out, Ls_res_out] = ...
    LOCAL_double_layer_splits(kout, t, geom_data);
  ...
  if ~printed_diagnostic
    fprintf('D-block Kress split diagnostics at N = %d:\n', ntot);
    fprintf('  L split residual, kin        : %.10e\n', L_res_in);
    fprintf('  L split residual, kout       : %.10e\n', L_res_out);
    fprintf('  L* split residual, kin       : %.10e\n', Ls_res_in);
    fprintf('  L* split residual, kout      : %.10e\n\n', Ls_res_out);
...
function [L1, L2, Ls1, Ls2, L_res, Ls_res] = ...
    LOCAL_double_layer_splits(k, t, geom_data)
  ...
  L_res = max(abs(L_full(offdiag) ...
    - L1(offdiag) .* logterm(offdiag) - L2(offdiag)));
  Ls_res = max(abs(Ls_full(offdiag) ...
    - Ls1(offdiag) .* logterm(offdiag) - Ls2(offdiag)));
