function run_tep_band_structure
% RUN_TEP_BAND_STRUCTURE Generate a draft band-structure plot for the star guide.
%
% Purpose:
...
%
% Main algorithm:
%   Sample a coarse beta-by-k grid, detect local k-dips for each fixed beta,
%   refine only those dips with deeper Kress discretizations, and collect
%   every evaluated (beta,k,sigma_min) point into one irregular data cloud for
%   scatter and contour-style visualization.
%
% Based on:
%   waveguide_1d/tep_plot_band.m, which uses the all-Kress matrix assembly
%   and the fixed-beta candidate refinement logic from tep_scan_global2.m.
%
% Main changes:
%   This draft example uses the Section 6.2 star inclusion and the draft MFS
%   parameters
%   \(H=0.6\), \(\delta_{\rm proxy}=0.15\), \(N_{\rm side}=160\),
%   \(N_{\rm top}=160\), \(N_{\rm proxy}=160\), and \(M_{\rm pw}=16\).
%   It writes numerical output under draft/examples/output, saves figures
%   under draft/figs, and leaves the final figures visible for inspection.
%
% Numerical goal:
...
refine_effort = 3;                  % Use all draft coarse/mid/final discretizations.
...
result_file = fullfile(output_dir, 'tep_band_structure_results.mat');
main_plot_file = fullfile(fig_dir, 'tep_band_structure.png');
scatter_plot_file = fullfile(fig_dir, 'tep_band_structure_scatter.png');
...
fprintf('  MFS Nside/Ntop/Nproxy/Mpw: %d / %d / %d / %d\n', ...
  pars2.N_side, pars2.N_top, 4 * pars2.N_proxy_edge, pars2.M_pw);
...
total_runtime_sec = toc(total_timer);
...
save(result_file, 'beta_interval', 'k_interval', 'beta_grid', 'ntot_list', ...
  ...
  'sigma_unique', 'final_candidates', 'total_runtime_sec');
...
fprintf('  total runtime              = %.3f s (%.3f min)\n', ...
...
function LOCAL_make_plots(beta_unique, k_unique, sigma_unique, beta_interval, k_interval, ...
  ...
  fig_scatter = figure('Name', 'TEP band diagram scatter', 'Color', 'w');
  ...
  grid on;
  exportgraphics(fig_scatter, scatter_plot_file, 'Resolution', 240);
  ...
  fig_main = figure('Name', 'TEP band diagram contour', 'Color', 'w');
  ...
  axis([beta_interval(1), beta_interval(2), k_interval(1), k_interval(2)]);
  grid on;
  exportgraphics(fig_main, main_plot_file, 'Resolution', 240);
  figure(fig_main);
  drawnow;
  shg;
