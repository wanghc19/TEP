...
\begin{cases}
  &\Delta u + k^2 u = 0, \quad (x, y) \in [X_\mathrm{L},X_\mathrm{R}] \times [0,d],\\
  &u|_{y = d} = e^{\ii\beta d} u|_{y = 0}, \quad \partial_y u|_{y = d} = e^{\ii\beta d} \partial_y u|_{y = 0}.
\end{cases}
\end{align*}
Define \begin{align*}
...
\end{align}
from which we can see that $G_\mathrm{QP}^{(k)}$ is not well-defined when $\gamma_m = 0$
for some $m$.
...
nontrivial Rayleigh-Bloch fields remain. We therefore write the
...
\begin{align}
  ...
  \end{cases}
\end{align}
where the Rayleigh-Bloch field $u_h$ has the form 
\begin{align}
  u_h[\xi](x, y) = \sum_{m \in \mathbb{Z}} \xi_m^\rightarrow e^{\ii \gamma_m (x - X^-)} \psi_m(y) + \sum_{m \in \mathbb{Z}} \xi_m^\leftarrow e^{-\ii \gamma_m (x - X^+)} \psi_m(y),\quad (x, y) \in \mathcal{C}\setminus \overline{\Omega}.  \label{eq:homogeneous-field}
\end{align}

\begin{conjecture}[Exact and unique single-cell representation]
\label{conj:single-cell-representation}
Let
$
  \mathcal C=[X_\mathrm{L},X_\mathrm{R}]\times[0,d]
$
be a periodic cell containing an inclusion $\Omega$, and assume that
$(k,\beta)$ is not at a Wood anomaly.
...
+D_{\rm QP}^{(k)}[\tau](x)
+u_h[\xi](x),
& x\in\mathcal C\setminus\overline{\Omega},\\[1mm]
S^{(nk)}[\sigma](x)
+D^{(nk)}[\tau](x),
& x\in\Omega,
\end{cases}
