% Purpose:
%   Fixed-beta global scan for transmission eigenvalue candidates using the
%   current all-Kress Muller matrix discretization.
%
% Main algorithm:
%   The script samples sigma_min(A(k)) on a coarse global k-grid, detects all
%   strict local dips, keeps a loose set of candidate dips, refines each
%   candidate at a mid resolution, then refines the best candidates again at
%   the final resolution.
%
% Based on:
%   The all-Kress matrix assembly used in tep_scan_local4_2.m and the fixed
%   beta scanning style of tep_scan_global.m.
%
% Main changes:
%   This is not a band-curve plotting script.  It uses all-Kress assembly for
%   S, D, D', and Tdiff, and it enforces a minimum refinement depth before any
%   relative-improvement stopping test is allowed.
%
% Numerical goal:
%   Find all local sigma_min dips for one fixed beta on a user-selected
%   k-interval and save the refined eigenvalue candidates.

function tep_scan_global2
...
ntot_mid = 60;                     % Mid-resolution refinement for every coarse candidate
ntot_final = 100;                  % Final refinement resolution for retained candidates

ngrid_global = 400;                % Number of global k samples
...
% --- 3. Stage 2: mid-resolution refinement ---
...
save(result_file, 'beta', 'k_interval', 'global_k_grid', 'global_sigma', ...
  'candidates', 'mid_results', 'final_results');
fprintf('\nSaved global scan results to %s\n', result_file);

if flag_plot_global
  LOCAL_plot_global_scan(global_k_grid, global_sigma, final_results, beta);
end
...
function LOCAL_plot_global_scan(k_grid, sigma_vals, final_results, beta)

  figure('Name', 'Fixed-beta Global Scan', 'Color', 'w');
