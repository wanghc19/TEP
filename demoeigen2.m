% Convergence Test for the FIRST Guided Mode (k) as ntot increases
format long;
clear;

% --- 1. Parameter Setup ---
ntot_list = [60, 80, 100, 120, 140, 160]; 
flag_geom = 'ellipse';
iprec = 10;
pars2.M = 200; % Increased M for higher Sl precision
pars2.L = 12;  % Increased L for better convergence floor
er = 13;
nref = sqrt(er);
d = 1.0;
beta = 0.5 * 2*pi/d; % Fixed Bloch phase (Gamma-X midpoint)

pars1.beta = beta;
pars1.d = d;

k1_history = zeros(length(ntot_list), 1);

fprintf('Starting Convergence Test for the FIRST mode (beta = %.4f)\n', beta);
fprintf('%-6s | %-18s | %-10s\n', 'ntot', 'Mode 1 (k)', 'Status');
fprintf('-------------------------------------------------------------\n');
...
for idx = 1:length(ntot_list)
  ...
  % Increase resolution to 100 points to avoid missing the first dip
  k_samples = linspace(0.01 * beta, 0.99 * beta, 100); 
  s_vals = zeros(size(k_samples));
  
  for j = 1:length(k_samples)
    s_vals(j) = LOCAL_get_min_sv(k_samples(j), nref, C, iprec, pars1, pars2, curvelen);
  end
  
  % --- 3. Robustly find the FIRST local minimum ---
  % We look for the first index where s_vals(j) < s_vals(j-1) and s_vals(j) < s_vals(j+1)
  target_idx = [];
  for j = 2:length(s_vals)-1
    if s_vals(j) < s_vals(j-1) && s_vals(j) < s_vals(j+1)
      target_idx = j;
      break; % Found the FIRST dip, stop here
    end
  end
  
  if isempty(target_idx)
    fprintf('%-6d | No mode found. Check bandgap or epsilon.\n', ntot);
    continue;
  end
  
  % --- 4. Refine using fminbnd with a tight bracket ---
  k_start = k_samples(target_idx - 1);
  k_end   = k_samples(target_idx + 1);
  
  obj_fun = @(k_val) LOCAL_get_min_sv(k_val, nref, C, iprec, pars1, pars2, curvelen);
  
  % Use high precision for root finding
  k_opt = fminbnd(obj_fun, k_start, k_end, optimset('TolX', 1e-13, 'Display', 'off'));
  
  k1_history(idx) = k_opt;
  fprintf('%-6d | %-18.12f | Success\n', ntot, k_opt);
end

% --- 5. Analysis of Convergence ---
valid_idx = find(k1_history > 0);
if length(valid_idx) > 1
  k_valid = k1_history(valid_idx);
  diffs = abs(diff(k_valid));
  fprintf('\nConvergence Analysis:\n');
  for i = 1:length(diffs)
    fprintf('ntot %d -> %d: |dk| = %e\n', ntot_list(i), ntot_list(i+1), diffs(i));
  end
end
...
function A = LOCAL_construct_A(C,iprec,khint,pars1,Sl,curvelen)
...
A = A + eye(2*ntot,2*ntot); % Add identity for better conditioning
