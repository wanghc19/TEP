...
@ARGV == 0 or die "run_preflight_002.pl accepts no arguments\n";

my $RUN_ID = 'resource-preflight-002';
...
my $BATCH = "run_i4_1b_core('resource-preflight-002','execution-003')";
my $start = time();
...
sub update_stage {
  ...
  while (my $line = <$handle>) {
    my $stage;
    if ($line =~ /^CORE_MARKER\t([^\t]+)\t(BUILD|REDUCE_K|REDUCE_M|SPD)_BEGIN/) {
      $stage = "$1/$2";
    } elsif ($line =~ /^CORE_MARKER\t([^\t]+)\t(RELEASED)/) {
      $stage = "$1/$2";
    }
    if (defined $stage) {
      ${$current} = $stage;
