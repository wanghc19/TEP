...
@ARGV == 0 or die "run_formal_002.pl accepts no arguments\n";

my $RUN_ID = 'run-002';
my $EXECUTION_ID = 'execution-002';
my $DEADLINE_EPOCH = 1788266751;
my $RSS_LIMIT_BYTES = 3221225472;
my $MATLAB = '/Applications/MATLAB_R2023b.app/bin/matlab';
my $BATCH = "run_i4_1b_core('run-002','execution-002')";
my $start = time();
...
my $draft = read_terminal(File::Spec->catfile(
  $output_dir, 'work', 'terminal.tsv'));
...
my $publication_ok = eval {
  ($summary_handle, $summary_partial, $elapsed_offset) =
    prepare_summary($draft, $peak_rss, $terminal);
  1;
};
if (!$publication_ok) {
  publication_failure('CANONICAL_PUBLICATION_FAILURE',
...
sub read_terminal {
  my ($path) = @_;
  my %values = (
    scientific_terminal => 'UNAVAILABLE', claim_boundary => 'UNAVAILABLE',
    attempted_solves => 0, completed_solves => 0,
    planned_solves => 0, lambda_pre => 'NaN', k_pre => 'NaN',
  );
  return \%values if !-e $path;
  open my $handle, '<', $path or return \%values;
  while (my $line = <$handle>) {
    chomp $line;
    my ($key, $value) = split /\t/, $line, 2;
    $values{$key} = $value if defined($value) && exists $values{$key};
  }
  close $handle;
  return \%values;
}
...
sub prepare_summary {
  my ($draft, $peak, $controller) = @_;
  my @columns = qw(run_id execution_id controller_terminal
    scientific_terminal claim_boundary attempted_solves completed_solves
    planned_solves lambda_pre k_pre elapsed_seconds aggregate_peak_rss_bytes);
  my $placeholder = '00000000000000000000';
  my @values = ($RUN_ID, $EXECUTION_ID, $controller,
    @{$draft}{qw(scientific_terminal claim_boundary attempted_solves
      completed_solves planned_solves lambda_pre k_pre)}, $placeholder, $peak);
  my $content = join(',', @columns) . "\n" . join(',', @values) . "\n";
  my $offset = index($content, $placeholder);
  my $partial = "$summary_path.partial";
  sysopen my $handle, $partial, O_WRONLY | O_CREAT | O_EXCL, 0600
    or die "cannot create run summary: $!\n";
  write_all($handle, $content);
  $handle->sync or die "cannot sync run summary partial: $!\n";
  !(-e $summary_path) or die "run summary already exists\n";
