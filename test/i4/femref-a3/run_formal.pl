...
my $BATCH = "run_i4_1c_core('run-001','execution-001')";
my $start = time();
...
sub update_stage {
  ...
  while (my $line = <$handle>) {
    ...
    if ($line =~ /^CORE_STAGE\t([^\t]+)\tBEGIN/) {
      $stage = $1;
    } elsif ($line =~ /^CORE_MARKER\t([^\t]+)\t([A-Z0-9_]+_(?:BEGIN|END)|RELEASED)/) {
      $stage = "$1/$2";
    }
    if (defined $stage) {
      ${$current} = $stage;
...
sub read_terminal {
  ...
  my %values = (
    ...
    planned_solves => 0, lambda_ref => 'NaN', k_ref => 'NaN',
  );
...
sub prepare_summary {
  ...
  my @columns = qw(run_id execution_id controller_terminal
    ...
    planned_solves lambda_ref k_ref elapsed_seconds aggregate_peak_rss_bytes);
  ...
  my @values = ($RUN_ID, $EXECUTION_ID, $controller,
    @{$draft}{qw(scientific_terminal claim_boundary attempted_solves
      completed_solves planned_solves lambda_ref k_ref)}, $placeholder, $peak);
