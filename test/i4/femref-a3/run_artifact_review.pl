#!/usr/bin/perl
# Purpose: Run the read-only I4.1c current-artifact inspector under the exact
# lifecycle wall/RSS predicates and publish only a create-once review log,
# resource ledger, and summary-last leaf.
use strict;
use warnings;
...
@ARGV == 0 or die "run_artifact_review.pl accepts no arguments\n";

my $RUN_ID = 'artifact-review-001';
...
my $BATCH = 'inspect_i4_1c_artifacts()';
my $start = time();
...
-d $run_dir or die "artifact-review run directory unavailable\n";
my $output_dir = File::Spec->catdir($run_dir, $EXECUTION_ID);
!(-e $output_dir) or die "OUTPUT_COLLISION: $RUN_ID/$EXECUTION_ID\n";
mkdir $output_dir or die "cannot claim artifact-review execution: $!\n";
my $log_path = File::Spec->catfile($output_dir, 'run.log');
$summary_path = File::Spec->catfile($output_dir, 'artifact-review-summary.tsv');
...
my $science_terminal = read_artifact_review_terminal($log_path);
if ($terminal eq 'NATURAL_EXIT' &&
    $science_terminal !~ /^(?:PASS|PASS_WITH_SCHEMA_CAVEATS)$/) {
  $terminal = 'ARTIFACT_REVIEW_OUTPUT_INCOMPLETE';
}
...
sub prepare_summary {
  ...
  sysopen my $handle, $partial, O_WRONLY | O_CREAT | O_EXCL, 0600
    or die "cannot create artifact-review summary: $!\n";
  write_all($handle, $content);
  $handle->sync or die "cannot sync artifact-review summary partial: $!\n";
  !(-e $summary_path) or die "artifact-review summary already exists\n";
