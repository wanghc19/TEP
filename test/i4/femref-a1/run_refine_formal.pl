...
@ARGV == 0 or die "run_refine_formal.pl accepts no arguments\n";
...
my $BATCH = "run_i4_1a_refine('run-008')";
...
if (!-e $run_dir) {
  mkdir $run_dir or die "cannot create output/run-008: $!\n";
}
-d $run_dir or die "output/run-008 is not a directory\n";
my $output_dir = File::Spec->catdir($run_dir, $EXECUTION_ID);
!(-e $output_dir) or
  die "OUTPUT_COLLISION: output/run-008/execution-001 exists\n";
mkdir $output_dir or
  die "cannot claim output/run-008/execution-001: $!\n";
...
sub write_summary {
  ...
  my @header = qw(run_id execution_id scientific_terminal terminal_class
    ...
    lambda_30 k_30 first_failure claim_boundary
