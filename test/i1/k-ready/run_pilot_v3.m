function result = run_pilot_v3()
%RUN_PILOT_V3 Run the bounded I1.4 V3 corrected-lift pilot.
% Purpose:
%   Execute the V2 affine family with only the V3 Dirichlet/Neumann lift
%   gate changed, preserving all earlier outputs.
% Output:
%   result - Evidence saved only under output/pilot-a3/result.mat.
% Main algorithm:
%   Evaluate the two frozen nodes through eval_k_v3 and retain the V3 lift
%   defects alongside the unchanged V2 scientific and engineering gates.
% Numerical goal:
%   Test the corrected lift consistency gate without any full-run work.

  if exist('OCTAVE_VERSION','builtin') ~= 0 || isempty(which('lsqminnorm'))
    error('kreadyv3:MATLABRequired','MATLAB with lsqminnorm is required.');
  end
  ...
  if numel(c.nodes) ~= 2 || numel(c.nodes) > c.pilot_node_limit
    error('kreadyv3:NodeLimit','V3 requires exactly the two frozen nodes.');
  end
  output = fullfile(here,'output','pilot-a3');
  if exist(output,'dir') || exist(output,'file')
    error('kreadyv3:OutputExists','output/pilot-a3 already exists.');
  end
  ...
  % --- stage 1: freeze V2 coordinates and apply the V3 lift gate ---
  ...
  if seed_pass
    point_timer = tic;
    complex_node = eval_k_v3('point',c.nodes(2),c,frame);
  ...
  if pass, status = 'I1_4_PILOT_V3_PASS'; ...
  else, status = 'I1_4_PILOT_V3_FAIL'; end
  ...
  fprintf('I1_K_READY_PILOT_V3 STATUS=%s pass=%d seconds=%.6g memory_mib=%.6g\n', ...
    status,pass,elapsed,retained_mib);
end

%% ==================== Parent and parity ====================
% These helpers are mechanically retained from V2.
...
function LOCAL_parent_gate(parent,c)
  ...
  if ~all(isfield(parent,required)) || ...
    ...
    error('kreadyv3:Parent','Frozen zoom2 parent lineage is unavailable.');
  end
  if parent.candidate.k ~= c.kstar || ...
      numel(parent.frame_rows_plus) ~= c.K || ...
      numel(parent.frame_rows_minus) ~= c.K
    error('kreadyv3:ParentContract','Frozen parent candidate or rows drifted.');
  end
end
...
%% ==================== Compact evidence ====================
% These helpers retain V2 ledgers and the four V3 lift quantities.
...
function rows = LOCAL_node_rows(node,index)
  rows = struct('node_index',{},'level',{},'k_real',{},'k_imag',{}, ...
    ...
    'dirichlet_defect',{},'neumann_defect',{},'lift_residual',{}, ...
    'kernel_defect',{});
  if isempty(node), return; end
  names = {'coarse','fine'};
  for j = 1:2
    x = node.(names{j}); p = x.participation;
    rows(end+1) = struct('node_index',index,'level',names{j}, ...
      ...
      'neumann_defect',p.neumann_defect,'lift_residual',p.residual, ...
...
% These helpers write source-closed, no-overwrite V3 evidence.

function p = LOCAL_provenance(here,repo,parent_path)
  names = {'plan-v3.md','cfg_v3.m','eval_k_v3.m','run_pilot_v3.m', ...
    'plan-v2.md','cfg_v2.m','eval_k_v2.m','kproxy_v2.m','kproxy.m', ...
    'kchan.m','kgreen.m','kbie.m'};
  ...
  if status ~= 0, error('kreadyv3:Git','Cannot record Git SHA.'); end
...
function value = LOCAL_hash(path)
  fid = fopen(path,'r');
  if fid < 0, error('kreadyv3:Hash','Cannot read %s.',path); end
...
function LOCAL_write_csv(path,rows)
  fid = fopen(path,'w');
  if fid < 0, error('kreadyv3:Artifact','Cannot write %s.',path); end
...
function LOCAL_write_report(path,r,c)
  ...
  fprintf(fid,'# I1-K-READY-PILOT-V3 report\n\n');
  fprintf(fid,'- Status: `%s`\n- Pass: `%d`\n',r.status,r.pass);
  fprintf(fid,'- Replacement lift threshold: `%.17g`\n',c.lift_residual_tol);
  ...
  fprintf(fid,'\n`kernel_defect` is diagnostic-only. No full work ran.\n');
  clear cleanup;
end
