function varargout = eval_k(action, varargin)
%EVAL_K Build the fixed-coordinate I1.4 homogeneous missing-column family.
% Purpose:
%   Freeze proxy/QZ charts at the real candidate and evaluate the same
%   branch-injected representation at prescribed complex wavenumbers.
% Input:
...
% Main algorithm:
%   Supply one anchored gamma branch to proxy, Green, incident, extractor,
%   center, and QZ consumers; freeze two-sided proxy SVD compression; freeze
%   QZ subspaces at the seed; continue off-seed clusters from generalized
%   right eigenspace overlap; reuse parent rows and the Dirichlet chart; form
%   unbalanced Adef D/G and a factor ledger.
% Based on:
%   test/i1/k-scan/zoom_core.m and the archived anchored readiness helpers.
% Main changes:
%   Removes pointwise square-root recreation, proxy lsqminnorm, QZ modulus
%   reselection, and k-varying analytic scoring from the positive path.
% Numerical goal:
%   Test sampled analytic-family readiness without locator or root work.
  ...
  elseif strcmp(action,'point')
    ...
    error('kready:Action','Unknown action %s.',action);
  end
end

%% ==================== Frozen representation ====================
% These helpers create branch anchors, proxy charts, and seed subspaces once.
...
function frame = LOCAL_seed_frame(c,rows)
  ...
  for j = 1:numel(c.levels)
    ...
    [~,A,~,~] = kproxy(pars,pspec,gamma,[],'collocation');
    [U,S,V] = svd(A,'econ'); singular = diag(S);
    rank_value = sum(singular/singular(1) >= c.proxy_rank_ratio);
    chart = struct('U',U(:,1:rank_value),'V',V(:,1:rank_value), ...
      'rank',rank_value,'singular_values',singular,'anchor',anchor);
    [U2,~,V2] = svd(A,'econ');
    chart.projector_repeat = max(LOCAL_projector(U(:,1:rank_value), ...
      U2(:,1:rank_value)),LOCAL_projector(V(:,1:rank_value), ...
      V2(:,1:rank_value)));
    if rank_value < numel(singular)
      chart.gap = singular(rank_value)/max(realmin,singular(rank_value+1));
    else
      chart.gap = Inf;
    end
    chart.rank_pass = chart.gap >= c.proxy_rank_gap_min || ...
      chart.projector_repeat <= c.proxy_projector_repeat_tol;
    chart.fingerprint = LOCAL_fingerprint([real(chart.U(:));imag(chart.U(:)); ...
      real(chart.V(:));imag(chart.V(:));rank_value]);
...
% These helpers build the two spatial levels with no hidden branch consumer.
...
function level = LOCAL_level(k,c,frame,index,is_seed)
  ...
  [proxy,Aproxy,~,pinfo] = ...
    kproxy(pars,pspec,proxy_gamma,chart,'collocation');
  proxy_full = pinfo.proxy_full_residual;
  [~,Aoff,boff,~] = kproxy(pars,pspec,proxy_gamma,[],'shifted');
  ...
  if ~isfinite(bie_rcond) || bie_rcond < c.bie_rcond_min
    error('kready:BIEFactor','A_QP failed the preregistered rcond gate.');
  end
  H = -(Aqp\rhs);
  ...
  factors = [ ...
    ...
    LOCAL_factor('proxy_full_diagnostic',pinfo.proxy_factor_rcond, ...
      max(proxy_full,off_residual),isfinite(off_residual),0,Inf); ...
  ...
  level = struct('name',spec.name,'k',k,'spec',spec,'gamma',port_gamma, ...
    ...
    LOCAL_branch_audit(k,proxy_anchor,c),'proxy_full_residual',proxy_full, ...
    'proxy_off_residual',off_residual,'factors',factors, ...
...
function q = LOCAL_qz_seed(A,B,K,c,plane)
  ...
  if numel(selected) ~= K
    error('kready:SeedCount','Seed %s stable count is %d, expected %d.', ...
      plane,numel(selected),K);
  end
  q = LOCAL_ordered_qz(A,B,raw,selected,K,c,plane);
  q.score_gap = Inf;
  q.chordal_separation = LOCAL_cross_chord(raw,find(stable),find(~stable));
...
function q = LOCAL_qz_continue(A,B,Zparent,K,c,plane)
  ...
  tau_cluster = max(c.cross_cluster_margin_min,10*max(raw.residual,q.residual));
  q.cluster_tau = tau_cluster;
  q.match_pass = score_gap > tau_cluster && ...
    classification_gap > tau_cluster && ...
    q.chordal_separation > tau_cluster && ...
...
function q = LOCAL_ordered_qz(~,~,raw,selected,K,c,plane)
  ...
  q = struct('plane',plane,'Z',Zs,'raw_residual',raw.residual, ...
    ...
    'indeterminate_count',sum(raw.indeterminate), ...
    'unit_gap',min(abs(raw.an(~raw.indeterminate)- ...
    raw.tn(~raw.indeterminate))),'pass',raw.residual <= c.qz_residual_tol && ...
...
function order = LOCAL_descending(scores)
  % Index is the deterministic tie breaker; no spectral label is presumed.
  [~,order] = sortrows([-scores(:),(1:numel(scores)).'],[1 2]);
end

%% ==================== Fixed chart and Adef ====================
% These helpers apply the parent row selectors and form both unbalanced forms.
...
function level = LOCAL_finish(level,c,frame)
  [Zp,pmargin,pcond,pres] = LOCAL_normalize(level.plus.Z,frame.rows_plus);
  [Zm,mmargin,mcond,mres] = LOCAL_normalize(level.minus.Z,frame.rows_minus);
  ...
  if ~(cp.safe && cm.safe)
    level.pass = false; level.reason = 'DIRICHLET_CHART_UNAVAILABLE'; return;
  end
  ...
  factors = [level.factors; ...
    LOCAL_factor('fixed_row_plus',rcond(level.plus.Z(frame.rows_plus,:)), ...
      pres,isfinite(pres),eps/c.chart_condition_eps_tol, ...
      c.small_solve_residual_tol); ...
    LOCAL_factor('fixed_row_minus',rcond(level.minus.Z(frame.rows_minus,:)), ...
      mres,isfinite(mres),eps/c.chart_condition_eps_tol, ...
      c.small_solve_residual_tol); ...
    cp.factor;cm.factor; ...
    LOCAL_factor('graph_schur_D_blocks',rcond(Cfactor),schur, ...
      isfinite(schur),c.dirichlet_rcond_min,c.schur_tol)];
...
function [Zhat,margin,condition,residual] = LOCAL_normalize(Z,rows)
...
function chart = LOCAL_chart(D,N,beta_m,c,name)
  ...
  chart = struct('margin',0,'condition',Inf,'solve_residual',Inf, ...
    ...
    c.factor_rcond_min,c.small_solve_residual_tol));
  ...
  if chart.margin <= c.chart_margin_min || ...
      chart.condition*eps > c.chart_condition_eps_tol, return; end
  chart.Lambda = (D.'\N.').';
  chart.solve_residual = norm(N-chart.Lambda*D,'fro')/max(1,norm(N,'fro'));
  chart.safe = chart.solve_residual <= c.small_solve_residual_tol;
  chart.factor = LOCAL_factor(name,rcond(D),chart.solve_residual, ...
