# Legacy route v1 eig-apost experiment index

## Archive record

- Pre-archive commit: `c05149a1a674d414b60a5d7d9429ca2dae51260c`.
- Archived on: `2026-08-11`.
- Scope: all experiments registered by the former eig-apost I0--I4 index.
- Preservation rule: scripts, configurations, reports, CSV/MAT/log outputs, manifests, hashes,
  absolute paths and the pre-archive index below remain frozen. Internal paths are not bulk-updated
  after the move.

The superseded part is the old finite-tail/remote-closure formulation, the associated augmented
coupling and its old root-readiness authorization chain. Reusable evidence remains conditional:
manufactured NEP algorithm tests, center-BIE/provenance diagnostics, Fliss parameter guidance,
Rayleigh-budget negatives, Linton-qualified Ewald value/derivative references, the frozen
three-path SLP/DLP action comparisons, the `proxy_dist/d=0.2` placement result, and the finite
$M_{\mathrm{trace}}=48$ screen. These results do not certify a current DtN, locator or root and
must be repeated when the geometry, solved density, trace space or solver contract changes.

The corresponding implementation archive is
