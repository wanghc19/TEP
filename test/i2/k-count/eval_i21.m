function varargout = eval_i21(action, varargin)
%EVAL_I21 Build the frozen fine-M48 I2.1 determinant family.
% Purpose:
%   Preserve the I1.4 anchored evaluator while exposing every matrix needed
%   for Method 1B factor-aware winding and gauge-free chart qualification.
% Input:
%   action - 'seed' or 'point'.
%   seed: cfg, parent row selectors. point: k, cfg, frozen frame.
% Output:
%   seed returns one fine node and the frozen frame; point returns one node.
% Main algorithm:
%   Solve the fine anchored seed proxy once with lsqminnorm, freeze
...
% Based on:
%   test/i1/k-ready/eval_k_v2.m and eval_k_v3.m.
% Main changes:
%   Remove the coarse level, expose R/A_QP/A/B/H/Z0/Dhat/AdefD, and apply
%   the corrected V3 Dirichlet/Neumann lift gate without changing formulas.
% Numerical goal:
%   Evaluate one fixed finite-dimensional family for a separate root count.

  action = lower(char(action));
  if strcmp(action,'seed')
    c = varargin{1}; rows = varargin{2};
    frame = LOCAL_seed_frame(c,rows);
    node = LOCAL_level(c.kstar,c,frame,true);
    frame.seed_Z_plus = node.plus.Z;
    frame.seed_Z_minus = node.minus.Z;
    node = LOCAL_finish(node,c,frame);
    frame.Z0_plus = node.Zp;
    frame.Z0_minus = node.Zm;
    varargout = {node,frame};
  elseif strcmp(action,'point')
    k = varargin{1}; c = varargin{2}; frame = varargin{3};
    node = LOCAL_level(k,c,frame,false);
    node = LOCAL_finish(node,c,frame);
    varargout = {node};
  else
    error('i21:EvaluatorAction','Unknown action %s.',action);
  end
end
...
function frame = LOCAL_seed_frame(c,rows)
  ...
  [~,A0,b0,~] = i21_kproxy(pars,pspec,gamma,[],'collocation');
  c0 = lsqminnorm(A0,b0);
...
% These helpers build the sole fine level and expose every true factor.

function level = LOCAL_level(k,c,frame,is_seed)
  timer = tic; spec = c.level; chart = frame.proxy_chart;
  ...
  [proxy,Aproxy,~,pinfo] = ...
    i21_kproxy(pars,pspec,proxy_gamma,chart,'collocation');
  [~,Aoff,boff,~] = i21_kproxy(pars,pspec,proxy_gamma,[],'shifted');
  off_residual = Inf;
  coefficients = pinfo.proxy_coefficients;
  if pinfo.available && ...
      isequal(size(coefficients),[size(Aoff,2),1]) && ...
      all(isfinite(coefficients))
    off_residual = norm(Aoff*coefficients-boff,2)/max(1,norm(boff,2));
  end
  proxy_gates = LOCAL_proxy_gates(is_seed,pinfo,chart,off_residual,c);
  proxy_hard = all([proxy_gates.available,proxy_gates.rank, ...
    proxy_gates.projected_residual,proxy_gates.projected_backward, ...
    proxy_gates.full_residual,proxy_gates.shifted_residual, ...
    proxy_gates.seed_identity]);
  if ~proxy_hard
    error('i21:ProxyGate','%s',LOCAL_proxy_gate_message( ...
      k,is_seed,pinfo,chart,off_residual,proxy_gates,c));
  end
  ...
  if ~isfinite(bie_rcond) || bie_rcond < c.bie_rcond_min
    error('i21:BIEFactor','A_QP failed the frozen rcond gate.');
  end
  H = -(Aqp\rhs);
  bie_residual = norm(Aqp*H+rhs,'fro')/max(1,norm(rhs,'fro'));
  if ~all(isfinite(H(:))) || bie_residual > c.bie_residual_tol
    error('i21:BIESolve','A_QP solve failed its residual gate.');
  end
  ...
  if is_seed
    ...
    plus = LOCAL_qz_continue(pair.A,pair.B,frame.seed_Z_plus, ...
      K,c,'LEFT_CELL_WALL');
    minus = LOCAL_qz_continue(pair.B,pair.A,frame.seed_Z_minus, ...
      K,c,'RIGHT_CELL_WALL');
  end
  factors = [ ...
  ...
  level = struct('name',spec.name,'k',k,'spec',spec,'gamma',port_gamma, ...
    ...
    'proxy_shifted_shape',size(Aoff),'Aqp_shape',size(Aqp),'branch_port', ...
    ...
    'R',pinfo.reduced_A,'Aqp',Aqp,'factors',factors, ...
...
function level = LOCAL_finish(level,c,frame)
  ...
  Gd = AdefG(drow,:); Gn = AdefG(nrow,:);
  dirichlet_defect = norm(Gd*lift,2)/max(1,norm(Gd,2)*norm(lift,2));
  neumann_defect = norm(Gn*lift-AdefD*q,2)/max(1, ...
    norm(Gn,2)*norm(lift,2)+norm(AdefD,2)*norm(q,2));
  kernel_defect = norm(AdefG*lift,2)/max(1, ...
    norm(AdefG,'fro')*norm(lift,2));
  level.H_plus_rows = frame.rows_plus;
  level.H_minus_rows = frame.rows_minus;
  level.participation = struct( ...
    ...
    'dirichlet_defect',dirichlet_defect, ...
    'neumann_defect',neumann_defect,'residual',max(dirichlet_defect, ...
    neumann_defect),'kernel_defect',kernel_defect);
...
function [Zhat,margin,condition,residual,rcond_value] = ...
  ...
  if margin <= c.chart_margin_min || ...
    ...
    error('i21:FixedRows','Fixed %s row factor failed closed.',side);
  end
  Zhat = (H.'\Z.').';
  residual = norm(Zhat(rows,:)-eye(size(H)),'fro')/max(1,sqrt(size(H,1)));
  if ~isfinite(residual) || residual > c.small_solve_residual_tol
    error('i21:FixedRows','Fixed %s row solve residual failed.',side);
  end
end
...
function value = LOCAL_projector(A,B)
  [Qa,~] = qr(A,0); [Qb,~] = qr(B,0);
  if ~isequal(size(Qa),size(Qb))
    value = Inf; return;
  end
  overlap = Qb'*Qa;
  value = norm(Qa-Qb*overlap,2);
end

function gates = LOCAL_proxy_gates(is_seed,pinfo,chart,off_residual,c)
  gates = struct( ...
    'available',logical(pinfo.available), ...
    'rank',logical(chart.rank_pass), ...
    'projected_residual',isfinite(pinfo.proxy_projected_residual) && ...
      pinfo.proxy_projected_residual <= c.proxy_projected_tol, ...
    'projected_backward',isfinite(pinfo.proxy_projected_backward) && ...
      pinfo.proxy_projected_backward <= c.proxy_projected_tol, ...
    'full_residual',isfinite(pinfo.proxy_full_residual) && ...
      pinfo.proxy_full_residual <= c.proxy_full_residual_max, ...
    'shifted_residual',isfinite(off_residual) && ...
      off_residual <= c.proxy_shifted_residual_max, ...
    'seed_identity',~is_seed || ...
      (isfinite(pinfo.proxy_seed_anchor_relative_error) && ...
      pinfo.proxy_seed_anchor_relative_error <= c.proxy_seed_identity_tol));
end

function message = LOCAL_proxy_gate_message(k,is_seed,pinfo,chart, ...
    off_residual,gates,c)
  reason = pinfo.failure_reason;
  if isempty(reason), reason = 'NONE'; end
  rcond_pass = isfinite(pinfo.proxy_factor_rcond) && ...
    pinfo.proxy_factor_rcond >= c.proxy_rcond_min;
  message = sprintf([ ...
    'Affine proxy hard gate failed: k_real=%.17g; k_imag=%.17g; ' ...
    'is_seed=%d; available=%d; failure_reason=%s; rank=%d; ' ...
    'rank_expected=%d; gap=%.17g; gap_min=%.17g; repeat=%.17g; ' ...
    'repeat_max=%.17g; rank_pass=%d; rcond=%.17g; rcond_min=%.17g; ' ...
    'rcond_pass=%d; projected_residual=%.17g; projected_residual_max=%.17g; ' ...
    'projected_residual_pass=%d; projected_backward=%.17g; ' ...
    'projected_backward_max=%.17g; projected_backward_pass=%d; ' ...
    'full_residual=%.17g; full_residual_max=%.17g; full_residual_pass=%d; ' ...
    'shifted_residual=%.17g; shifted_residual_max=%.17g; ' ...
    'shifted_residual_pass=%d; seed_identity_enabled=%d; ' ...
    'seed_identity=%.17g; seed_identity_max=%.17g; seed_identity_pass=%d.'], ...
    real(k),imag(k),is_seed,gates.available,reason,chart.r,c.expected.proxy_rank, ...
    chart.gap,c.proxy_rank_gap_min,chart.projector_repeat, ...
    c.proxy_projector_repeat_tol,gates.rank,pinfo.proxy_factor_rcond, ...
    c.proxy_rcond_min,rcond_pass,pinfo.proxy_projected_residual, ...
    c.proxy_projected_tol,gates.projected_residual, ...
    pinfo.proxy_projected_backward,c.proxy_projected_tol, ...
    gates.projected_backward,pinfo.proxy_full_residual, ...
    c.proxy_full_residual_max,gates.full_residual,off_residual, ...
    c.proxy_shifted_residual_max,gates.shifted_residual,is_seed, ...
    pinfo.proxy_seed_anchor_relative_error,c.proxy_seed_identity_tol, ...
    gates.seed_identity);
end
