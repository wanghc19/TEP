function [proxy, proxy_A, proxy_b, info] = ...
    kproxy(pars1, pars2, branch_gamma, ...
    proxy_chart, point_set)
%KPROXY Assemble and solve one anchored proxy system.
%
% Purpose:
...
%
% Numerical goal:
%   Provide one analytic proxy representation throughout the I1.4 disk.
  ...
  elseif strcmp(point_set, 'shifted')
    ...
    error('kready:InvalidProxyPointSet', ...
  ...
  for idx = 1:length(required)
    if ~isfield(proxy_chart, required{idx})
      error('kready:InvalidProxyChart', ...
